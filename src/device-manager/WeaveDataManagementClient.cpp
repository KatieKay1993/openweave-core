...
WEAVE_ERROR GenericTraitUpdatableDataSink::SetStringArray(const char * apPath, const std::vector<std::string> & aValueVector, bool aIsConditional)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    nl::Weave::TLV::TLVWriter writer;
    PropertyPathHandle propertyPathHandle = kNullPropertyPathHandle;
        ...
        case SubscriptionClient::kEvent_OnSubscribeRequestPrepareNeeded:
            ...
            VerifyOrExit(kOpState_RefreshData == savedOpState, err = WEAVE_ERROR_INCORRECT_STATE);
            {
                ...
                if (needSubscribeSpecificTrait) {
                    pathListLen = 1;
                    err = pWdmClient->mSinkCatalog.PrepareSubscriptionSpecificPathList(pWdmClient->mpPublisherPathList,
                                                                                       traitListLen,
                                                                                       handle);
                ...
                {
                    err = pWdmClient->mSinkCatalog.PrepareSubscriptionPathList(pWdmClient->mpPublisherPathList,
                                                                                                traitListLen,
                                                                                                pathListLen);
