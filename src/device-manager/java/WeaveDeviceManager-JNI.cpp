...
static std::string J2N_StdString(JNIEnv *env, jstring inString);
...
std::string J2N_StdString(JNIEnv *env, jstring inString)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    ...
    ret = std::string((char *)pDataBuf, length);
...
void Java_nl_Weave_DataManagement_GenericTraitUpdatableDataSinkImpl_setStringArray(JNIEnv *env, jobject self, jlong genericTraitUpdatableDataSinkPtr, jstring path, jobjectArray stringArray, jboolean isConditional)
{
    ...
    {
        jstring jst = (jstring) (env->GetObjectArrayElement(stringArray, i));
        std::string val = J2N_StdString(env, jst);
        stringVector.push_back(val);
        env->DeleteLocalRef(jst);
        VerifyOrExit(val != "", err = WEAVE_ERROR_NO_MEMORY);
    }
